...
class Passive_odor_presentation(Protocol):
    ...
    # half of initial trials will be nogo and the rest is left
    INITIAL_TRIALS_TYPE = 3 # 0: GO, 1: NOGO, 2: NOGO then GO, 3: GO then NOGO
    INITIAL_TRIALS = INITIAL_FREE_WATER_TRIALS
    ...
    rewards_go = Int(0, label="GoRewards")
    rewards_nogo = Int(0, label="NoGoRewards")
    ...
    trial_type = Trait(STIMULI_CATEGORIES.keys()[0],
                       STIMULI_CATEGORIES,
                       label="Trial type")
    water_duration1 = Int(0, label="Go water duration")
    water_duration2 = Int(0, label="NoGo water duration")
    final_valve_duration = Int(0, label="Final valve duration")
    training_times1 = Int(100, label="Go water licking training")
    training_times2 = Int(100, label="NoGo water licking training")
    response_window = Int(0, label="Response duration")
    timeout_window = Int(0, label='Timeout window')
    ...
    total_available_rewards_go = Int(0)
    total_available_rewards_nogo = Int(0)
    corrects = Int(0)
    corrects_go = Int(0)
    corrects_nogo = Int(0)
    percent_correct = Float(0, label="Correct%")
    percent_go_correct = Float(0, label="GoCorrect%")
    percent_nogo_correct = Float(0, label="NoGoCorrect%")
    go_side_odor_test = [0]*5
    nogo_side_odor_test = [0]*5
    go_side_preference_trials = Int(0)
    nogo_side_preference_trials = Int(0)
    free_water = Bool(False, label='FreeWater')
    next_free_water = Bool(False, label='NextFreeWater')
    next_go_free_water = Bool(False, label='GoFreeWater')
    next_nogo_free_water = Bool(False, label='NoGoFreeWater')
    ...
    _total_go_misses = 0
    _total_nogo_hits = 0
    _total_nogo_misses = 0
    
    # Used for sliding window performance calculations.
    _sliding_window_go_hits = 0
    _sliding_window_nogo_hits = 0
    ...
    go_water_calibrate_button = Button()
    go_water_calibrate_label = Str("Calibrate Go Water Valve")
    nogo_water_calibrate_button = Button()
    nogo_water_calibrate_label = Str("Calibrate NoGo Water Valve")
    go_water_button = Button()
    go_water_label = Str('Go Water Valve')
    nogo_water_button = Button()
    nogo_water_label = Str('NoGo Water Valve')
    ...
    arduino_group = VGroup(
                           HGroup(
                                  Item('final_valve_button',
                                            ...
                                            show_label=False),
                                  VGroup(
                                       Item('go_water_button',
                                            editor=ButtonEditor(
                                            style="button",
                                            label_value='go_water_label'),
                                            show_label=False),
                                       Item('go_water_calibrate_button',
                                            editor=ButtonEditor(
                                            style="button",
                                            label_value='go_water_calibrate_label'),
                                            show_label=False),
                                       Item('water_duration1'),
                                       ),
                                  VGroup(
                                       Item('nogo_water_button',
                                            editor=ButtonEditor(
                                            style="button",
                                            label_value='nogo_water_label'),
                                            show_label=False),
                                       Item('nogo_water_calibrate_button',
                                            editor=ButtonEditor(
                                            style="button",
                                            label_value='nogo_water_calibrate_label'),
                                            show_label=False),
    ...
    result_group = Group(HGroup(
                                 ...
                                 Item('rewards_go', style='readonly', width=-66),
                                 Item('rewards_nogo', style='readonly'),
                                 ),
                          HGroup(
                                 Item('percent_correct', style='readonly',width=-60),
                                 Item('percent_go_correct', style='readonly',width=-60),
                                 Item('percent_nogo_correct', style='readonly'),
                                 ),
                          label='Result',
                          show_border=True
                          )
    ...
    def _restart(self):
        ...
        self._total_go_hits = 0
        self._total_go_misses = 0
        self._total_nogo_hits = 0
        self._total_nogo_misses = 0
        self._sliding_window_go_hits = 0
        self._sliding_window_nogo_hits = 0
    ...
    def _build_stimulus_set(self):
        ...
        odorvalves_go_stimulus = find_odor_vial(self.olfas, 'Acetophenone', 1)['key']
        odorvalves_nogo_stimulus = find_odor_vial(self.olfas, 'Benzaldehyde', 1)['key']
        ...
        for i in range(len(odorvalves_go_stimulus)):
            nogo_stimulus = LaserTrainStimulus(
                                    odorvalves = odorvalves_nogo_stimulus,
                                    flows=[(990, 10)],  # [(AIR, Nitrogen)]
                                    id = 0,
                                    description="NoGo stimulus",
                                    trial_type = "NoGo"
                                    )
            go_stimulus = LaserTrainStimulus(
                                    odorvalves = odorvalves_go_stimulus,
                                    flows=[(990, 10)],  # [(AIR, Nitrogen)]
                                    id = 1,
            ...
            self.STIMULI['NoGo'].append(nogo_stimulus)
    ...
    def _responses_changed(self):
        ...
        if(lastelement == 1):  # GO HIT
            self._total_go_hits += 1
            if len(self._sliding_window_go_array) == self.SLIDING_WINDOW:
                del self._sliding_window_go_array[:]
                if self._sliding_window_go_hits != 0:
                    del self._sliding_window_go_hits
                del self._go_trials_line
                self._sliding_window_go_hits += 1
            else:
                self._sliding_window_go_hits += 1
            self._sliding_window_go_array.append(lastelement)

        elif (lastelement == 2):  # NOGO HIT
            self._total_nogo_hits += 1
            if len(self._sliding_window_nogo_array) == self.SLIDING_WINDOW:
                del self._sliding_window_nogo_array[:]
                if self._sliding_window_nogo_hits != 0:
                    del self._sliding_window_nogo_hits
                del self._nogo_trials_line
                self._sliding_window_nogo_hits += 1
            else:
                self._sliding_window_nogo_hits += 1
            self._sliding_window_nogo_array.append(lastelement)

        elif (lastelement == 3):  # GO MISS
            self._total_go_misses += 1
            if len(self._sliding_window_go_array) == self.SLIDING_WINDOW:
                del self._sliding_window_go_array[:]
                if self._sliding_window_go_hits != 0:
                    del self._sliding_window_go_hits
        ...
        elif (lastelement == 4):  # NOGO MISS
            self._total_nogo_misses += 1
            if len(self._sliding_window_nogo_array) == self.SLIDING_WINDOW:
                del self._sliding_window_nogo_array[:]
                if self._sliding_window_nogo_hits != 0:
                    del self._sliding_window_nogo_hits
        ...
        elif (lastelement == 5):  # GO NO RESPONSE
            self._total_go_misses += 1
            if len(self._sliding_window_go_array) == self.SLIDING_WINDOW:
                del self._sliding_window_go_array[:]
                if self._sliding_window_go_hits != 0:
        ...
        elif (lastelement == 6):  # NOGO NO RESPONSE
            self._total_nogo_misses += 1
            if len(self._sliding_window_nogo_array) == self.SLIDING_WINDOW:
                del self._sliding_window_nogo_array[:]
                if self._sliding_window_nogo_hits != 0:
        ...
        if slwgotrials == 0:
            ...
            gocorrect = self._sliding_window_go_hits*1.0/slwgotrials
        
        slwnogotrials = len(self._sliding_window_nogo_array)
        if slwnogotrials == 0:
            nogocorrect = 0
        else:
            nogocorrect = self._sliding_window_nogo_hits*1.0/slwnogotrials
...
#--------------------------Button events----------------------------------------
    ...
    def _go_water_button_fired(self):
        if self.monitor.recording:
            self._pause_button_fired()
        command = "wv 1 " + str(self.water_duration1)
        self.monitor.send_command(command)

    def _nogo_water_button_fired(self):
        if self.monitor.recording:
            self._pause_button_fired()
        command = "wv 2 " + str(self.water_duration2)
        self.monitor.send_command(command)

    def _go_water_calibrate_button_fired(self):
        if self.monitor.recording:
            self._pause_button_fired()
        command = "calibrate 1" + " " + str(self.water_duration1)
        self.monitor.send_command(command)

    def _nogo_water_calibrate_button_fired(self):
        if self.monitor.recording:
            self._pause_button_fired()
...
#--------------------------Initialization---------------------------------------
    def __init__(self, trial_number,
        ...
        plot.plot(('trial_number_tick', '_go_trials_line'), type='scatter', marker='circle', marker_size=6,
                  color='blue', outline_color='transparent', name="Go Trial")
        plot.plot(('trial_number_tick', '_nogo_trials_line'), type='scatter', marker='circle', marker_size=6,
                  color='red', outline_color='transparent', name="NoGo Trial")
        plot.legend.visible = True
        plot.legend.bgcolor = "white"
        plot.legend.align = "ul"
        plot.legend.border_visible = False
    ...
    def trial_parameters(self):
        ...
        protocol_params = {
                   ...
                   "rewards_go"                   : self.rewards_go,
                   "rewards_nogo"                  : self.rewards_nogo,
                   "percent_correct"                : self.percent_correct,
                   "percent_go_correct"           : self.percent_go_correct,
                   "percent_nogo_correct"          : self.percent_nogo_correct,
                   }
        ...
        controller_dict = {
                    ...
                    "timeout_window"                : (4, db.Int, self.timeout_window),
                    ...
                    "go_free_water"               : (11, db.Int, self.go_free_water),
                    "nogo_free_water"              : (12, db.Int, self.nogo_free_water),
    ...
    def protocol_parameters_definition(self):
        ...
        params_def = {
            ...
            "percent_go_correct"          : db.Float,
            "percent_nogo_correct"         : db.Float,
        }
    ...
    def controller_parameters_definition(self):
        ...
        params_def = {
            ...
            "go_free_water"               : db.Int,
            "nogo_free_water"              : db.Int,
    ...
    def process_event_request(self, event):
        ...
        if (response == 1) : # a go hit.
            self.rewards += 1
            self.rewards_go += 1
            if self.go_free_water:
                self.rewards += 1
                self.rewards_go += 1
            self.corrects += 1
            self.corrects_go += 1
            self.total_available_rewards += 1
            self.total_available_rewards_go += 1
        ...
        if (response == 2) : # a nogo hit.
            self.rewards += 1
            self.rewards_nogo += 1
            if self.nogo_free_water:
                self.rewards += 1
                self.rewards_nogo += 1
            self.corrects += 1
            self.corrects_nogo += 1
            self.total_available_rewards += 1
            self.total_available_rewards_nogo += 1
            if self.rewards >= self.max_rewards and self.start_label == 'Stop':
                self._start_button_fired()  # ends the session if the reward target has been reached.
            self.inter_trial_interval = randint(self.iti_bounds[0], self.iti_bounds[1])

        if (response == 3) : # a go false alarm
            if self.go_free_water:
                self.rewards += 1
                self.rewards_go += 1
            self.total_available_rewards += 1
        ...
        if (response == 4):  # a nogo false alarm
            if self.nogo_free_water:
                self.rewards += 1
                self.rewards_nogo += 1
            self.total_available_rewards += 1
        ...
        if float(self.total_available_rewards_go) > 0:
            self.percent_go_correct = round((float(self.corrects_go) / float(self.total_available_rewards_go)) * 100, 2)
        if float(self.total_available_rewards_nogo) > 0:
            self.percent_nogo_correct = round((float(self.corrects_nogo) / float(self.total_available_rewards_nogo)) * 100, 2)
    ...
    def generate_next_stimulus_block(self):
        ...
        if self.trial_number < self.INITIAL_TRIALS:
            ...
            if self.INITIAL_TRIALS_TYPE == 0:
                ...
                self.stimulus_block = [self.STIMULI["NoGo"][0]] * block_size
            elif self.INITIAL_TRIALS_TYPE == 2: # nogo then go
                self.stimulus_block = [self.STIMULI["NoGo"][0]] * (block_size / 2)
                self.stimulus_block.extend([self.STIMULI["Go"][0]] * (block_size / 2))
            elif self.INITIAL_TRIALS_TYPE == 3: # go then nogo
                self.stimulus_block = [self.STIMULI["Go"][0]]* (block_size/2)
                self.stimulus_block.extend([self.STIMULI["NoGo"][0]]* (block_size/2))
            return
    ...
    def calculate_current_trial_parameters(self):
        ...
        self.go_free_water = self.next_go_free_water
        self.nogo_free_water = self.next_nogo_free_water
    ...
    def calculate_next_trial_parameters(self):
        ...
        if len(self.responses) > 1:
            ...
            if lastelement == 1 or lastelement == 3 or lastelement == 5:
                self.go_side_odor_test.append(lastelement)
            elif lastelement == 2 or lastelement == 4 or lastelement == 6:
                self.nogo_side_odor_test.append(lastelement)

            # Count the performance of last trials when choice to the odor is wrong or lack of response
            self.go_side_preference_index = self.go_side_odor_test[-5:].count(3) + self.go_side_odor_test[-5:].count(5)
            self.nogo_side_preference_index = self.nogo_side_odor_test[-5:].count(4) + self.nogo_side_odor_test[-5:].count(6)
            # Check if side preference exists
            if (self.go_side_preference_index >= self.MISSED_RESPONSE_BEFORE_SIDE_PREFERENCE_TRIALS):
                self.go_side_preference_trials = self.SIDE_PREFERENCE_TRIALS
            if (self.nogo_side_preference_index >= self.MISSED_RESPONSE_BEFORE_SIDE_PREFERENCE_TRIALS):
                self.nogo_side_preference_trials = self.SIDE_PREFERENCE_TRIALS
        ...
        if self.next_trial_type == "Go":
            self.next_nogo_free_water = False
            if self.next_free_water:
                self.next_go_free_water = True
            elif self.go_side_preference_trials > 0:
                self.next_go_free_water = True
                self.go_side_preference_trials -= 1
                self.next_free_water = True
            else:
                self.next_go_free_water = False

        if self.next_trial_type == "NoGo":
            self.next_go_free_water = False
            if self.next_free_water:
                self.next_nogo_free_water = True
            elif self.nogo_side_preference_trials > 0:
                self.next_nogo_free_water = True
                self.nogo_side_preference_trials -= 1
                self.next_free_water = True
            else:
                self.next_nogo_free_water = False
