...
const PROGMEM unsigned int reward_on = {1};
...
unsigned long mri_onset = 0;
unsigned long hrf_phase = 0;
...
unsigned long random_probability = 0;
...
void loop() {
  ...
  if (pulse_on && (totalms - last_pulse > tr)) {
	...
	case 2: // Wait for hrf phase time to be over and exhalation state.
  	    if ((digitalRead(MRI_TRIGGER) != mri_trigger_state)) {
          mri_onset = totalms;
          if ((totalms-mri_onset) > (hrf_phase)) {
              state = 3;
          }
  	    }
	    break;
	...
	case 4: // Wait for hrf phase time to be over and inhalation state.
        if ((digitalRead(MRI_TRIGGER) != mri_trigger_state)) {
            mri_onset = totalms;
            if ((totalms-mri_onset) > (hrf_phase)) {
              state = 5;
